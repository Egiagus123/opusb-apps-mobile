...
class AuthCubit extends Cubit<AuthState> {
  ...
  Future<void> loadAuthorizationData(
      {required int clientId, int roleId = 0, int orgId = 0}) async {
    ...
    try {
      ...
      loadOrg(clientId: clientId, roleId: roleId, roles: roles);
    } catch (e) {
  ...
  Future<void> loadOrg(
      ...
      required int roleId,
      required List<IdNamePair> roles,
      int orgId = 0}) async {
    log.i('loadOrgs(clientId: $clientId , role : $roleId)');

    try {
      emit(AuthInProgress());
      final List<IdNamePair> orgs =
          await sl<AuthService>().getOrg(clientId, roleId);
      orgId = orgId == 0 ? orgs.first.id : orgId;
  ...
  Future<void> authorize(int clientId, int roleId, int orgId) async {
    log.i('authorize(clientId:$clientId , roleId:$roleId, orgId:$orgId)');
    try {
      emit(AuthInProgress());
      final AuthResponse response = await sl<AuthService>().authorize(
          LoginParameters(
              clientId: clientId, roleId: roleId, organizationId: orgId));
      await _checkVersion();
